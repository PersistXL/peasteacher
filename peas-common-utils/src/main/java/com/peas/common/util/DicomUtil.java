...
/**
 * Created by duan on 2016/1/6.
 */
public class DicomUtil {

    public static boolean isDicomFile(String filePath) throws FileNotFoundException {
        return isDicomFile(new FileInputStream(filePath));
    }

    public static boolean isDicomFile(File file) throws FileNotFoundException {
        return isDicomFile(new FileInputStream(file));
    }

    public static boolean isDicomFile(byte[] header) {
        return isDicomFile(new ByteArrayInputStream(header));
    }

    private static boolean isDicomFile(InputStream file) {
        int rlen = 0;
        byte[] b128 = null;
        try {
            b128 = new byte[128];
        ...
        return (rlen >= 8 && (guessTransferSyntax(b128, rlen, false) || guessTransferSyntax(b128, rlen, true)));
    }

    private static boolean guessTransferSyntax(byte[] b128, int rlen, boolean bigEndian) {
        int tag1 = ByteUtils.bytesToTag(b128, 0, bigEndian);
        VR vr = vrOf(tag1);
        if (vr == VR.UN) {
            return false;
        } else if (ByteUtils.bytesToVR(b128, 4) == vr.code()) {
            return true;
        } else {
    ...
    public static VR vrOf(int tag) {
        if ((tag & '\uffff') == 0) {
            ...
            return (tag & '\uff00') == 0 && (tag & 240) != 0 ? VR.LO : VR.UN;
        } else if ((tag & -256) == 2109696) {
            return VR.CS;
        } else {
            int tmp = tag & -2097152;
            if (tmp != 1342177280 && tmp != 1610612736) {
                if ((tag & -16777216) == 2130706432 && (tag & -65536) != 2145386496) {
                    tag &= -16711681;
                }
            } else {
                tag &= -2031617;
    ...
    public static class ByteUtils {
        ...
        public static int bytesToTagLE(byte[] bytes, int off) {
            return (bytes[off + 1] << 24) + ((bytes[off] & 255) << 16) + ((bytes[off + 3] & 255) << 8) + (bytes[off + 2] & 255);
        }
    }
