 ...
 */
public class HsfAntPathMatcher
{
     ...
     * cache off completely. <p>Default is for the cache to be on, but with the
     * variant to automatically turn it off when encountering too many patterns
     * to cache at runtime (the threshold is 65536), assuming that arbitrary
     * permutations of patterns are coming in, with little chance for
     * encountering a reoccurring pattern.
     ...
     * settings. <p>Performs caching based on {@link #setCachePatterns},
     * delegating to {@link #tokenizePath(String)} for the actual tokenization
     * algorithm.
     ...
     * little chance for encountering a reoccurring pattern. <p>This method may
     * get overridden to implement a custom cache strategy.
     * @param pattern the pattern to match against (never {@code null})
     ...
     * <p>For example: <ul> <li>'{@code /docs/cvs/commit.html}' and
     * '{@code /docs/cvs/commit.html} -> ''</li> <li>'{@code /docs/*}' and
     * '{@code /docs/cvs/commit} -> '{@code cvs/commit}'</li>
     * <li>'{@code /docs/cvs/*.html}' and '{@code /docs/cvs/commit.html} ->
     * '{@code commit.html}'</li> <li>'{@code /docs/**}' and
     * '{@code /docs/cvs/commit} -> '{@code cvs/commit}'</li>
     * <li>'{@code /docs/**\/*.html}' and '{@code /docs/cvs/commit.html} ->
     * '{@code cvs/commit.html}'</li> <li>'{@code /*.html}' and
     * '{@code /docs/cvs/commit.html} -> '{@code docs/cvs/commit.html}'</li>
     * <li>'{@code *.html}' and '{@code /docs/cvs/commit.html} ->
     * '{@code /docs/cvs/commit.html}'</li> <li>'{@code *}' and
     * '{@code /docs/cvs/commit.html} -> '{@code /docs/cvs/commit.html}'</li>
     * </ul> <p>Assumes that {@link #match} returns {@code true} for
     * '{@code pattern}' and '{@code path}', but does <strong>not</strong>
     * enforce this.
     */
    ...
    /**
     * Combines two patterns into a new pattern that is returned. <p>This
     * implementation simply concatenates the two patterns, unless the first
     * pattern contains a file extension match (such as {@code *.html}. In that
     * case, the second pattern should be included in the first, or an
     * {@code IllegalArgumentException} is thrown. <p>For example: <table>
     * <tr><th>Pattern 1</th><th>Pattern 2</th><th>Result</th></tr>
     * <tr><td>/hotels</td><td>{@code
     * null}</td><td>/hotels</td></tr>
     ...
     * patterns in order of explicitness. <p>The returned {@code Comparator}
     * will
     * {@linkplain Collections#sort(List, Comparator)
     ...
     * following patterns: <ol> <li>{@code /hotels/new}</li>
     * <li>{@code /hotels/{hotel}}</li> <li>{@code /hotels/*}</li> </ol> the
     * returned comparator will sort this list so that the order will be as
     * indicated. <p>The full path given as parameter is used to test for exact
     * matches. So when the given path is {@code /hotels/2}, the pattern
     * {@code /hotels/2} will be sorted before {@code /hotels/1}.
     ...
     * {@link Pattern}. <p>The pattern may contain special characters: '*' means
     * zero or more characters; '?' means one and only one character; '{' and
     * '}' indicate a URI template pattern. For example <tt>/users/{user}</tt>.
     */
    protected static class AntPathStringMatcher
    {
        ...
        public boolean matchStrings(String str, Map<String, String> uriTemplateVariables)
        {
            ...
            if (matcher.matches())
            {
                ...
                {
                    // SPR-8455
                    Preconditions.checkState(this.variableNames.size() == matcher.groupCount(),
